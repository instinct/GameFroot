...
-(int) dpadDirection
{
	int x = self.stickPosition.x > 0 ? 1 : -1;
	if(self.stickPosition.x <= self.deadRadius && self.stickPosition.x >= -self.deadRadius)		x = 0;
	
	int y = self.stickPosition.y > 0 ? 1 : -1;
	if(self.stickPosition.y <= self.deadRadius && self.stickPosition.y >= -self.deadRadius)		y = 0;
	
	int dir = DPAD_NONE;
	
	if(x ==  1 && y ==  0)		dir = DPAD_RIGHT;
	if(x == -1 && y ==  0)		dir = DPAD_LEFT;
	if(x ==  0 && y ==  1)		dir = DPAD_UP;
	if(x ==  0 && y == -1)		dir = DPAD_DOWN;
	
	if(x ==  1 && y == -1)		dir = DPAD_DIAG_DR;
	if(x == -1 && y == -1)		dir = DPAD_DIAG_DL;
	if(x ==  1 && y ==  1)		dir = DPAD_DIAG_UR;
	if(x == -1 && y ==  1)		dir = DPAD_DIAG_UL;
